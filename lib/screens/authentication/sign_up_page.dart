...
import '../../network/models/loginInfo.dart';
            ...
            child: SingleChildScrollView(
              child: Column(
                children: [
                  ...
                  //   Provider.of<LoginInfoProvider>(context).createLoginInfo(LoginInfo.guest(isSynced: false));
...
class _SignUpFormState extends State<SignUpForm> {
  ...
  void _onSignUp() async {
    try {
      var userCredentials = await FirebaseAuth.instance.createUserWithEmailAndPassword(email: emailController.text.trim(), password: passwordController.text.trim());
      ...
      await FirebaseFirestore.instance.collection('userNotes').doc(userCredentials.user!.email).set(data);
      print('user stored in database');
      var document = await FirebaseFirestore.instance.collection('userNotes').doc(userCredentials.user!.email).get();
      var userData = document.data()!;
      print('user got fetched');

      if(!context.mounted) {
        throw Exception("can't sign up, context not available error");
      }

      userData = {'type': 'user', 'userDetails': {'name': userData['name'], 'email': userData['email'], 'password': userData['password'], 'isSynced': userData['isSynced']}};
      if(_isMerging) {
      ...
      throw Exception('firebase firestore exception : $e');
  ...
  Widget build(BuildContext context) {
    ...
    return Form(
      key: _formKey, // Form key to access the form state
      child: Column(
        children: [
          TextFormField(
            onSaved: (username) {},
          ...
          ElevatedButton(
            onPressed: () async {
              if (_formKey.currentState!.validate()) {
                // Save the form if all validators pass
